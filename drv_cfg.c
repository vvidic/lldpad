...
static int recv_msg(int cmd, int attr, unsigned int seq)
{
	...
	if ((d->cmd != cmd) || (rta->rta_type != attr)) {
		TRACE("Bad netlink message attribute.");
		return -EIO;
	}
	...
	struct feature_support *dcb_capabilities)
{
	...
	for (i = 0; rta_parent > rta_child; i++) {
		...
		case DCB_CAP_ATTR_GSP:
			...
			break;
		default:
			printf("unknown capability %d: %02x\n",
...
int get_hw_state(char *ifname, int *dcb_state)
{
	...
	if (err)
		TRACE("Adapter does not support DCB.\n");
	else
		TRACE("Adapter supports DCB.\n");
...
int set_hw_state(char *ifname, int dcb_state)
{
	...
	if (err)
		printf("ERROR %s: set_linkmode dcbstate %i\n",
			__func__, dcb_state);
...
int get_perm_hwaddr(const char *ifname, u8 *buf_perm, u8 *buf_san)
{
	...
	if (rta->rta_type != DCB_ATTR_PERM_HWADDR) {
		...
		LLDPAD_ERR("%s:%s:expecting PERM_HWADDR but got 0x%x\n",
			   __func__, ifname, rta->rta_type);
		rval = -EIO;
		goto get_error;
	}

	memcpy(buf_perm, NLA_DATA(rta), ETH_ALEN);
	memcpy(buf_san, NLA_DATA(rta) + ETH_ALEN, ETH_ALEN);

	LLDPAD_DBG("%s:%s:LAN MAC=%2x:%2x:%2x:%2x:%2x:%2x\n",
		   __func__, ifname,
		   buf_perm[0], buf_perm[1], buf_perm[2],
		   buf_perm[3], buf_perm[4], buf_perm[5]);

	LLDPAD_DBG("%s:%s:SAN MAC=%2x:%2x:%2x:%2x:%2x:%2x\n",
		   __func__, ifname,
		   buf_san[0], buf_san[1], buf_san[2],
		   buf_san[3], buf_san[4], buf_san[5]);
...
bool check_port_dcb_mode(char *ifname)
{
	...
	if (dcb_state) {
		TRACE2("config.c: %s dcb mode is ON.\n", ifname);
		return true;
	} else {
		TRACE2("config.c: %s dcb mode is OFF. \n", ifname);
		return false;
	}
}
